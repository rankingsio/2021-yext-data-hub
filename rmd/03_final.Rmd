...
Visually, there does not seem to be a large correlation between the _impressions_ and the _clicks_. However, the Pearson correlation coefficient is $r = `r round(rcor, 2)`$. In other words, there is some correlation between the number of impressions and the clicks. This is driven by the large common increase in fall 2021. However, this results might be due to the fact that only a few clicks were collected for the Legal Services, only an average of 2 to 4 per month. The data might be too small for such a statistical comparison.   
...
The _Average Extended Network Impressions for Longtail Keywords_ is for _all industries together_ is `r round(avg_impressions_allindustry/avg_impressions_legalservices, 1)` times larger than for _legal services_. The gap is even larger for the _Clicks_, it is `r round(avg_clicks_allindustry/avg_clicks_legalservices, 1)` times larger for _all industries together_ than for _legal services_. This suggests that _legal services_ have generally less success in turning impressions into clicks (for this sample of listings).  
